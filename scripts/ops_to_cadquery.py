...
def _default_py_path(ops_json_path: str) -> str:
    base_dir = os.path.dirname(os.path.abspath(ops_json_path))
    base_name = os.path.splitext(os.path.basename(ops_json_path))[0]
    return os.path.join(base_dir, f"{base_name}_cq.py")
...
def main():
    ...
    # Output: always save in same folder (even if --out_py not provided)
    def _default_py_path(json_path):
        base_dir = os.path.dirname(os.path.abspath(json_path))
        base_name = os.path.splitext(os.path.basename(json_path))[0]
        return os.path.join(base_dir, f"{base_name}_cq.py")

    out_path = args.out_py or _default_py_path(args.ops_json)
    os.makedirs(os.path.dirname(os.path.abspath(out_path)), exist_ok=True)
    with open(out_path, "w") as f:
        f.write(code)
    if args.debug:
        print(f"Wrote CadQuery code → {out_path}")

    # Also print to stdout (unchanged behavior)
